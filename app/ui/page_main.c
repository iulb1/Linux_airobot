...
static menu_info_t menu_info_list[] = {
    ...
    // {MENU_LANGUAGE,"小游戏",GET_IMAGE_PATH("icon_language.png")}, 空间不足，已删除音频
    ...
    // {MENU_BT_AUDIO,"蓝牙音频",GET_IMAGE_PATH("icon_menu_bt_audio.png")}, 蓝牙音箱和小智未做兼容
    {MENU_AI_XIAOZHI,"AI小智",GET_IMAGE_PATH("icon_menu_ai_xiaozhi.png")},
...
static void menu_click_event_cb(lv_event_t * e){
    ...
    if(strcmp(menu_name,"场景联动\0") == 0){
        ...
        delete_current_page(&com_style);
        init_page_about_us();
    } else if(strcmp(menu_name,"蓝牙音频\0") == 0){
        delete_current_page(&com_style);
        init_page_bt_audio();
    }else if(strcmp(menu_name,"AI小智\0") == 0){
        delete_current_page(&com_style);
        init_page_ai();
    }
}
...
void init_page_main(void)
{
    ...
    if(page_type == TIME_TYPE_2){
        ...
        lv_gif_set_src(gif_obj, GET_IMAGE_PATH("earth01.gif"));
